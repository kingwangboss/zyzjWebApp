...
export default {
  ...
  methods: {
    payClick() {
      ...
      if (this.isSelect == 1) {
        console.log("微信");
        // this.url = 'http://wz.camew.com/wechat/purchase/#/wxservicePrice/' + this.pid + '?uid=' + localStorage.uid;
        let data = new FormData();
        data.append("pid", this.pid);
        data.append("uid", localStorage.uid);
        data.append("paytype", 1);
        this.$http
          .post("http://wz.camew.com/home/PayPost", data)
          .then(res => {
            // console.log(res);
        ...
        // this.url = 'http://wz.camew.com/wechat/purchase/#/servicePrice/' + this.pid + '?uid=' + localStorage.uid;
        let data = new FormData();
        data.append("pid", this.pid);
        data.append("uid", localStorage.uid);
